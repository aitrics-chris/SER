...
def set_model(args, device_id, _idx, _model, _linear_classifier, _optimizer, _scheduler, _gradscaler):
    ...
    if args.arch[_idx] == 'vit_small':
        ...
        _model = vits.__dict__[args.arch[_idx]](patch_size=args.patch_size, num_classes=0, equiv_layer=int(args.equiv_layer[_idx]), if_baseline=args.if_baseline[_idx].upper() == 'TRUE')  
    ...
    if 'moco' in args.ckpts[_idx]:
        ...
        for k in list(state_dict.keys()):
            ...
            if args.if_baseline[_idx].upper() == 'TRUE':
                if ('norm_equiv' in k):
                    del state_dict[k]
    elif 'barlowtwins' in args.ckpts[_idx]:
        state_dict = torch.load(args.ckpts[_idx], map_location='cpu', weights_only=False)['model']
    
    missing_keys, unexpected_keys = _model.load_state_dict(state_dict, strict=True)
    ...
    folder = os.path.join('results', args.ckpts[_idx].split('/')[-2]+f'_seed{args.seed}_epoch{args.epochs}{args.tag}')
...
def eval_linear(args):
    ...
    for epoch in tqdm(range(0, args.epochs)):
        ...
        if epoch >= args.val_epoch:
            acc1_list_tensor = validate(args, val_loader, epoch) # scoring, save ckpt

            for idx_process in range(args.process_num):
                if (acc1_list_tensor[idx_process] > args.best_acc1[idx_process]):
                    args.best_epoch[idx_process] = epoch + 1
                    args.best_acc1[idx_process] = acc1_list_tensor[idx_process]
                    save_dict(args, idx_process, epoch)
                    
            acc1_list = [_acc1.cpu().item() for _acc1 in acc1_list_tensor]
            acc1_str = ', '.join(str(_acc1) for _acc1 in acc1_list)
    ...
    acc1_list_tensor_test = validate(args, test_loader, -99)
    acc1_list_test = [_acc1.cpu().item() for _acc1 in acc1_list_tensor_test]

    for idx_process in range(args.process_num):  
        with open(args.txt_name[idx_process] + '.txt', 'a' if os.path.isfile(args.txt_name[idx_process] + '.txt') else 'w') as f:
            f.write(f'\nlr_init: {args.lrs[idx_process]}, best_acc1_val: {args.best_acc1[idx_process]}, acc1_test: {acc1_list_test[idx_process]}, best_epoch: {args.best_epoch[idx_process]}')
    ####################### end get test acc #######################

def save_dict(args, idx_process, epoch):
    _save_dict = {
        "epoch": epoch + 1,
        ...
        "best_acc": args.best_acc1[idx_process],
        "best_epoch": args.best_epoch[idx_process],
    }
    torch.save(_save_dict, args.txt_name[idx_process] + f'_lr_{args.lrs[idx_process]}.pth.tar')
...
def validate(args, loader, epoch):
    ...
    def _worker(_image, _target, _model, _linear_classifier, _optimizer, _scaler, _gpu, _stream, i):
    ...
    for (image, target) in loader:
        ...
        threads = [
                        threading.Thread(
                            target=_worker, args=(image, target, args.models[i], args.linear_classifiers[i], args.optimizers[i], args.gradscalers[i], args.gpus[i], streams[i], i)
                        )
                        # for i, (module, input, kwargs, device, stream) in enumerate(
                        #     zip(args.models, args.linear_classifiers, args.optimizers, args.schedulers, inputs, n_last_blocks, devices, streams)
                        # )
        ...
        return [correct/all for correct, all in zip(num_correct, num_all)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='/home/chris/storage/imagenet_eval', type=str)
    parser.add_argument('--test_path', default='/mnt/aitrics_ext/ext01/chris/imagenet/val', type=str)
