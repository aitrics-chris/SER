...
from builder.utils import trunc_normal_
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, equiv_layer=12, equiv_mode='inv', ssl_type='moco', **kwargs):
        super().__init__()
        ...
        if equiv_mode != 'erl':
            ...
            self.forward_inv = self.forward_inv_
            self.get_intermediate_layers = self.get_intermediate_layers_equiv
    ...
    def interpolate_pos_encoding_baseline(self, x, w, h):
        npatch = x.shape[1] - 1
    ...
    def forward_inv_(self, x):
        # print(f'forward_inv_')
        with torch.autocast(device_type="cuda"):
            h = int(x.size(2)/16)
            w = int(x.size(3)/16)
    ...
    # @torch.no_grad()
    def get_intermediate_layers_equiv(self, x, n=1):
        # print(f'get_intermediate_layers_equiv')
        # with torch.autocast(device_type="cuda"):
        x = self.prepare_tokens(x)
    ...
    # @torch.no_grad()
    def get_intermediate_layers_baseline(self, x, n=1):
        # print(f'get_intermediate_layers_baseline')
        # with torch.autocast(device_type="cuda"):
        # x = self.prepare_tokens(x)
        B, nc, _w, _h = x.shape
        x = self.patch_embed(x)  # patch linear embedding
...
# def vit_tiny(args, patch_size=16args, **kwargs):
